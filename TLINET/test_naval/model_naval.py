...
def neural_sort(s, tau):
    device = s.device
    n = s.size()[0]
    one = torch.ones((n, 1), device=device, dtype=s.dtype)  # Ensure same dtype as s
    A_s = torch.abs(s.unsqueeze(-1) - s.unsqueeze(-2))
    B = torch.matmul(A_s, torch.matmul(one, one.transpose(0, 1)))
    scaling = (n + 1 - 2 * (torch.arange(n, device=device, dtype=s.dtype) + 1)).unsqueeze(0)
    C = torch.matmul(s.unsqueeze(-1), scaling)
    P_max = C - B
...
def smooth_quantile(scores, quantile, temperature):
    ...
    P_hat = neural_sort(scores[0], temperature) 
    sorted_scores = torch.matmul(P_hat, scores.T).squeeze(-1)  
    N = scores.size(1)
    quantile_index = quantile * (N - 1)
    idx = torch.arange(N, device=device).float()
    weights = F.softmax(-torch.abs(idx - quantile_index) / temperature, dim=0)
    ...
    for epoch in range(1, n_iters):
        ...
        if epoch % 100 == 0:
            ...
            if val_acc_bnl >= acc_best:
                ...
                #pcb_val = pcb.clone().detach()
                #pdb_val = pdb.clone().detach()
